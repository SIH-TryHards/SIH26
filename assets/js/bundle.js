(() => {
  ...
  var en = {
    ...
    "adv.heat.body": "Maximum temperature is around {tmax}\xB0C. Irrigate before 8 am and avoid spraying between 11 am and 4 pm.",
    ...
    "officer.jurisdictionLine": "Agriculture officer \xB7 {district} district",
    ...
    "distress.rainHint": "0 mm dry \xB7 100 mm flood risk",
    "distress.priceLabel": "Mandi price drop vs average",
    "distress.priceHint": "0% stable \xB7 50% sharp fall",
    "distress.loanLabel": "Days until next EMI",
    "distress.loanHint": "0 days due now \xB7 120 days safe",
  ...
  var te = {
    ...
    "distress.scaleHigh": "High Distress",
    "distress.rainHint": "mm of rain in 3 days",
    "distress.priceLabel": "Compared with MSP",
    "distress.priceHint": "% below MSP",
    "distress.loanHint": "days until due",
    "distress.high": "High Risk",
    "distress.low": "Normal",
  ...
  function saveMandiPreferences(patch) {
  ...
  var mandiCropSelect = createSelect({
  ...
  function applyTheme() {
    ...
    if (value)
      value.textContent = theme === "dark" ? t("profile.darkMode") : t("profile.lightMode");
    if (button)
      button.textContent = theme === "dark" ? t("profile.useLightMode") : t("profile.useDarkMode");
  }
  ...
  function selectLanguage(language, { advance }) {
    ...
    if (advance) {
      ...
      if (["home", "mandi", "help", "profile", "loan", "officer"].includes(currentView)) {
        if (currentView === "home" && getSession())
          renderHome().catch(() => {
          });
  ...
  function applyCopy() {
    ...
    $("officerCallBtn").innerHTML = `${phone(16)}<span>${escapeHtml(t("help.callBtn"))}</span>`;
    ...
    $("kccCallBtn").innerHTML = `${phone(15)}<span>${escapeHtml(t("help.kccPhone"))}</span>`;
    $("disasterName").textContent = t("help.disasterLine");
    $("disasterHours").textContent = t("help.disasterSupport");
    $("disasterCallBtn").innerHTML = `${phone(15)}<span>${escapeHtml(t("help.disasterPhone"))}</span>`;
    $("helpKccQuickCall").innerHTML = `${phone(15)}<span>${escapeHtml(t("help.kccQuickCall"))}</span>`;
    $("helpPmkisanPortal").innerHTML = `${building(15)}<span>${escapeHtml(t("help.pmkisanPortal"))}</span><span aria-hidden="true">\u2197</span>`;
  ...
  async function onSubmitProfile(event) {
    ...
    try {
      ...
      saveDraftProfile({ name, email: email || null, contactPreference: $("profileContactInput").value, attached: true });
      go("home");
    } catch (e) {
      ...
      <div class="phenology-header">
        <div class="phenology-title-wrap">
          <span class="phenology-eyebrow">
            ${sprout(16)} ${escapeHtml(t("pheno.title"))}
          </span>
  ...
  function renderSprayWindow(weather) {
    ...
    const statusLabel = spray.status === "optimal" ? t("spray.optimal") : spray.status === "caution" ? t("spray.caution") : t("spray.unsafe");
    const defaultSlots = spray.status === "optimal" ? [{ start: "06:00 AM", end: "09:30 AM", period: t("spray.morningCalm") }, { start: "05:00 PM", end: "07:00 PM", period: t("spray.dusk") }] : spray.status === "caution" ? [{ start: "06:00 AM", end: "08:00 AM", period: t("spray.earlyOnly") }] : [];
    ...
    <div class="spray-card">
      <div class="spray-header">
        <div class="phenology-title-wrap">
          <span class="phenology-eyebrow">
            ${sprayer(16)} ${escapeHtml(t("spray.title"))}
          </span>
        ...
        <div class="spray-slots-list">
          ...
          `).join("") : `
            <span class="spray-slot-pill" style="border-color:hsl(var(--danger)/.3); color:hsl(var(--danger));">
              ${xCircle(13)} ${escapeHtml(t("spray.suspended"))}
            </span>
          `}
        </div>
      </div>
  ...
  function renderTomorrowActionPlan(draft, weather, pheno) {
    ...
    <div class="action-plan-card">
      <div class="phenology-header">
        <div class="phenology-title-wrap">
          <span class="phenology-eyebrow">
            ${activity(16)} ${escapeHtml(t("tomorrow.title"))}
          </span>
  ...
  function paintAdvisoryDetailListen(speaking) {
    ...
    btn.dataset.speaking = String(speaking);
    btn.innerHTML = speaking ? `${stopIcon(16)}<span>${escapeHtml(t("home.stop"))}</span>` : `${speaker(16)}<span>${escapeHtml(t("home.listen"))}</span>`;
  }
  function onListen() {
    if (isSpeaking()) {
      stop();
  ...
  function onAdvisoryDetailListen() {
    ...
    const p = renderParams(a.params);
    const locale = getLanguageByCode(getLang())?.locale ?? "en-IN";
    const result = speak([t(a.titleKey, p), t(a.bodyKey, p), t(a.whyKey, p)], locale, {
  ...
  async function renderMandi(overrideQty) {
    const draft = getDraftProfile() ?? {};
    const preferences = getMandiPreferences?.() ?? {};
    const selectedCrop2 = preferences.crop ?? meCache?.crop_cycle?.crop ?? draft.crop ?? "cotton";
    mandiCropSelect.setOptions(CROPS.map((crop) => ({ value: crop.value, label: t(crop.key) })));
    mandiCropSelect.setValue(CROPS.some((crop) => crop.value === selectedCrop2) ? selectedCrop2 : CROPS[0].value);
    const eff = {
      ...draft,
      ...
      crop: mandiCropSelect.getValue() || selectedCrop2
    };
  ...
  function renderHelp() {
    ...
    $("officerDistrict").textContent = `${district} \xB7 ${t("help.state") || "Maharashtra"}`;
  ...
  function wire() {
    ...
    document.querySelectorAll("[data-loan-date]").forEach((button) => {
      button.addEventListener("click", () => {
        const date = /* @__PURE__ */ new Date();
        if (button.dataset.loanDate === "next")
          date.setMonth(date.getMonth() + 1, 1);
        if (button.dataset.loanDate === "today")
          date.setDate(date.getDate());
        if (button.dataset.loanDate === "october")
          date.setMonth(9, 1);
    ...
    $("clearAllDataBtn").addEventListener("click", () => {
      if (!window.confirm(t("loan.clearDataConfirm")))
        return;
      clearAllData();
      meCache = null;
